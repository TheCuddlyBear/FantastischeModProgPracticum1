...
public partial class Form1 : Form {
  ...
  public Form1() {
    ...
    Bitmap bmp =
        GenMandelImage(panel1.Size, midX, midY, scaleFactor, maxIterations);
  ...
  public Bitmap GenMandelImage(Size size, float midX, float midY,
                               float scaleFactor, int maxIterations) {
    ...
    for (int pixelY = 0; pixelY < size.Height; pixelY++) {
      for (int pixelX = 0; pixelX < size.Width; pixelX++) {
        ...
        do {
          (a, b) = (a * a - b * b + x,
                    2 * a * b + y); // tuple for generating a and b
          iteration++;
        } while (iteration <= maxIterations && a * a + b * b < 4);
        if (iteration < maxIterations) {
          Complex zn = new Complex(a, b);
          double colorAlgorithm =
              iteration + 1 -
              Math.Log(Math.Log(Complex.Abs(zn)) / Math.Log(2)) / Math.Log(2);
          ...
          Color color = linearInterpolation(color1, color2, (int)(colorAlgorithm
          % 1));*/
          // // Continuous Smooth coloring algorithm (see wikipedia)
          Color color =
              Utils.ColorFromHSV(0.95 + 5 * colorAlgorithm, 0.6,
                                 1.0); // Generating the color from HSV values
  ...
  private void button1_Click(object sender, EventArgs e) {
    ...
    if (string.IsNullOrEmpty(midXTb.Text) ||
        string.IsNullOrEmpty(midYTb.Text) |
            string.IsNullOrEmpty(maxIterationsTb.Text) ||
        string.IsNullOrEmpty(scaleFactorTb.Text)) {

      MessageBox.Show("Je hebt niet alle velden ingevuld!", "Error!",
  ...
  private void saveCurrentImageToolStripMenuItem_Click(object sender,
                                                       EventArgs e) {
    Utils.SavePictureBoxImage(panel1); // See Utils class for the code
  }

  private void infoOverMandelbrotToolStripMenuItem_Click(object sender,
                                                         EventArgs e) {
  ...
  private void panel1_MouseClick(object sender, MouseEventArgs e) {
    float midX = string.IsNullOrEmpty(midXTb.Text)
                     ? Properties.Settings.Default.midX
                     : float.Parse(midXTb.Text);
    float midY = string.IsNullOrEmpty(midYTb.Text)
                     ? Properties.Settings.Default.midY
                     : float.Parse(midYTb.Text);
    float pixelX = e.X;
    float pixelY = e.Y;

    if (e.Button == MouseButtons.Right) {

      float oldScaleFactor = string.IsNullOrEmpty(scaleFactorTb.Text)
                                 ? Properties.Settings.Default.scaleFactor
                                 : float.Parse(scaleFactorTb.Text);
      ...
      int maxIter = string.IsNullOrEmpty(maxIterationsTb.Text)
                        ? Properties.Settings.Default.maxIter
                        : int.Parse(maxIterationsTb.Text);
